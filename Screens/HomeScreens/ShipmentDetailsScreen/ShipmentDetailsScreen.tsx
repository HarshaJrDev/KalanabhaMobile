...
// ─── Status config ──────────────────────────────────────────────────────────────
const STATUS: Record<string, { label: string; color: string; bg: string; emoji: string }> = {
    delivered: { label: 'Delivered', color: C.success, bg: C.successLight, emoji: '✅' },
    'in-transit': { label: 'In Transit', color: C.primary, bg: C.primaryLight, emoji: '🚛' },
    pending: { label: 'Pending', color: C.warning, bg: C.warningLight, emoji: '⏳' },
    active: { label: 'Active', color: C.primary, bg: C.primaryLight, emoji: '📦' },
    cancelled: { label: 'Cancelled', color: C.danger, bg: C.dangerLight, emoji: '❌' },
};

// ─── Timeline steps ─────────────────────────────────────────────────────────────
const TIMELINE = [
    { key: 'placed', label: 'Order Placed', emoji: '📋', done: true },
    { key: 'picked', label: 'Picked Up', emoji: '📦', done: true },
    { key: 'in-transit', label: 'In Transit', emoji: '🚛', done: true },
    { key: 'out', label: 'Out for Delivery', emoji: '🏍️', done: false },
            ...
            >
                ...
                <View style={styles.headerTop}>
                    ...
                    <TouchableOpacity
                        style={styles.backBtn}
                        activeOpacity={0.8}
                        onPress={() => Alert.alert('Share', 'Sharing tracking link…')}
                    >
                            ...
                            <View style={styles.timelineLeft}>
                                {isActive ? (
                                    ...
                                    <View style={[styles.tlDot, isDone && styles.tlDotDone]}>
                                        <Text style={{ fontSize: isDone ? 12 : 10 }}>{isDone ? step.emoji : '○'}</Text>
                                    </View>
                                )}
                                {!isLast && (
        ...
        <AnimatedCard anim={cardAnims[1]} fade={cardFades[1]}>
            ...
            <View style={styles.routeWrap}>
                ...
                <View style={styles.routeConnector}>
                    ...
                    <LinearGradient colors={[C.primary, C.accent]} style={styles.routeArrowBadge} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
    ...
    const renderPackage = () => (
        <AnimatedCard anim={cardAnims[2]} fade={cardFades[2]}>
            ...
            <View style={styles.packageGrid}>
                {[
                    { emoji: '🏷️', label: 'Name', value: data?.name || DEMO.name },
                    { emoji: '📁', label: 'Category', value: data?.category || DEMO.category },
                    { emoji: '⚖️', label: 'Weight', value: data?.weight || DEMO.weight },
                    { emoji: '🔢', label: 'Quantity', value: String(data?.quantity ?? DEMO.quantity) },
                ].map(item => (
                    <View key={item.label} style={styles.packageItem}>
                        <Text style={styles.pkgEmoji}>{item.emoji}</Text>
    ...
    const renderActions = () => (
        <AnimatedCard anim={cardAnims[4]} fade={cardFades[4]} noPad>
            <View style={styles.actionsGrid}>
                {[
                    { emoji: '🗺️', label: 'Live Map', color: [C.primary, '#6366F1'] as const },
                    { emoji: '📞', label: 'Call Driver', color: ['#10B981', '#059669'] as const },
                    { emoji: '💬', label: 'Chat Support', color: ['#F59E0B', '#D97706'] as const },
                    { emoji: '📄', label: 'Download POD', color: ['#EF4444', '#DC2626'] as const },
                ].map(btn => (
                    <TouchableOpacity key={btn.label} style={styles.actionBtn} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
    ...
    pkgEmoji: { fontSize: 22, marginBottom: H(6) },
